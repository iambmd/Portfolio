...
function initCursor(): void {
  ...
  const follower = document.getElementById("cursorFollower") as HTMLElement;
  ...
  const hoverTargets = document.querySelectorAll<HTMLElement>(
    "a, button, .project-item, .skill-tag, .social-link"
  );
...
// ─── COUNTER ANIMATION ────────────────────────
function animateCounter(el: HTMLElement, target: number, duration = 1400): void {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const heroEls = document.querySelectorAll<HTMLElement>(
    ".hero .reveal, .hero .reveal-delay-1, .hero .reveal-delay-2, .hero .reveal-delay-3"
  );
